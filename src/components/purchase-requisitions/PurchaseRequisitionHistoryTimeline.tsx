...
export function PurchaseRequisitionHistoryTimeline({ history }: Props) {
  ...
  return (
    <div className="space-y-4 text-xs">
      <h4 className="font-bold uppercase tracking-wider text-slate-500 text-xs">
        Línea de Tiempo del Requerimiento ({history.length})
      </h4>
        ...
        {history.map((item) => (
          <div key={item.id} className="relative flex flex-col gap-1 rounded-xl border border-slate-200 bg-white p-3.5 shadow-xs">
            ...
            <div className="flex items-center justify-between text-[11px] text-slate-400">
              <span className="font-mono font-bold text-slate-700">{item.action}</span>
