...
const ITEMS: Array<{ label: string; path: string; icon: LogisticsIconName; end?: boolean }> = [
  { label: 'Resumen', path: '/logistics/putaway', icon: 'dashboard', end: true },
  { label: 'Órdenes', path: '/logistics/putaway/orders', icon: 'package' },
  { label: 'Tareas', path: '/logistics/putaway/tasks', icon: 'check-square' },
  { label: 'Excepciones', path: '/logistics/putaway/exceptions', icon: 'alert' },
  { label: 'Capacidad', path: '/logistics/putaway/capacity', icon: 'layers' },
  { label: 'Móvil', path: '/logistics/putaway/mobile', icon: 'sessions' },
]

export function PutawayPhaseNav() {
  return (
    <nav aria-label="Navegación de ubicación dirigida" className="overflow-x-auto rounded-2xl border border-slate-200 bg-white p-1.5 shadow-sm">
